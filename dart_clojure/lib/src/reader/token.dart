...
enum TokenType {
  ...
  string, number, character, boolean, nil,
  symbol, keyword,
  quote, deref, // リーダーマクロ
  dispatch, // #{}(セット), #"regex", #'var, #()匿名関数, #_無視
  syntaxQuote, unquote, unquoteSplicing,
  comment, eof
}
...
class Tokenizer {
  ...
  void _scanToken() {
    ...
    switch (char) {
      ...
      case '': _addToken(TokenType.rightBracket); break;
      case '{': _addToken(TokenType.leftBrace); break;
      case '}': _addToken(TokenType.rightBrace); break;
      case "'": _addToken(TokenType.quote); break;
      case '@': _addToken(TokenType.deref); break;
      case '`': _addToken(TokenType.syntaxQuote); break;
      case '~':
        if (_match('@')) {
      ...
      default:
        if (_isDigit(char)) {
          ...
          // エラー処理: 予期しない文字を報告
          print("Error: Unexpected character '$char' at line $_line, column ${_current - _lineStart}");
        }
    }
  }
  ...
  void _number() {
    ...
    } else if (_peek() == 'N' && (_isAtEndNext() ||!_isAlphaNumeric(_peekNext())) ) { // BigInt
        _advance(); // N を消費
  ...
  bool _isAlphaNumeric(String char) => _isAlpha(char) |
| _isDigit(char);
...
| _peek() == '/') {
  ...
  void _character() {
    ...
    // より複雑なケース: \newline, \u0041 など
    String charLiteral;
    if (_current + "newline".length <= source.length && source.substring(_current, _current + "newline".length) == "newline") {
        _current += "newline".length;
        charLiteral = "\n";
    } else if (_current + "space".length <= source.length && source.substring(_current, _current + "space".length) == "space") {
        _current += "space".length;
        charLiteral = " ";
    } else if (_current + "tab".length <= source.length && source.substring(_current, _current + "tab".length) == "tab") {
        _current += "tab".length;
        charLiteral = "\t";
    }
    // \uXXXX と \oNNN の解析を追加
